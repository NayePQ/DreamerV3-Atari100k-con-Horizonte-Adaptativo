...
class Agent(embodied.jax.Agent):
  ...
  def __init__(self, obs_space, act_space, config):
    ...
    self.imag_dyn_ema = nj.Variable(
        jnp.array, float(config.imag_adapt.abs_thresh), f32, name='imag_dyn_ema')
  ...
  def loss(self, carry, obs, prevact, training):
    ...
    imag_mask, imag_mets = self._imag_mask(losses['dyn'], H, training)
    ...
    metrics.update(mets)
    metrics.update(prefix(imag_mets, 'imagadapt'))
  ...
  def _imag_mask(self, dyn_loss, horizon, training):
    mask = jnp.ones((1, horizon), f32)
    cfg = self.config.imag_adapt
    if not cfg.enabled:
      return mask, {}

    dyn_now = sg(f32(dyn_loss.mean()))
    ema_prev = f32(self.imag_dyn_ema.read())
    ema_new = (1 - cfg.ema_rate) * ema_prev + cfg.ema_rate * dyn_now
    if training:
      self.imag_dyn_ema.write(ema_new)
    ema = ema_new if training else ema_prev

    step = f32(self.opt.step.read())
    warmed = step >= cfg.warmup
    rel = dyn_now / jnp.maximum(f32(cfg.eps), ema)
    bad = warmed & (dyn_now > cfg.abs_thresh) & (rel > cfg.rel_thresh)
    sev = jnp.clip(
        (rel - cfg.rel_thresh) /
        jnp.maximum(f32(cfg.eps), f32(cfg.rel_max - cfg.rel_thresh)),
        0.0, 1.0)
    gamma = 1.0 - sev * (1.0 - cfg.min_decay)
    time = jnp.arange(horizon, dtype=f32)
    soft = gamma ** time
    soft = jnp.where(time < cfg.min_horizon, 1.0, soft)
    soft = jnp.maximum(soft, f32(cfg.tail_floor))
    mask = jnp.where(bad, soft, jnp.ones_like(soft))[None, :]
...
def imag_loss(
  ...
  weight_policy = weight[:, :-1]
  if imag_mask is not None:
    assert imag_mask.ndim == 2, imag_mask.shape
    assert imag_mask.shape[-1] == weight_policy.shape[-1], (
        imag_mask.shape, weight_policy.shape)
    weight_policy = weight_policy * imag_mask
  policy_loss = sg(weight_policy) * -(
  ...
  metrics['weight'] = weight_policy.mean()
